...
export const DownloadScreen = React.memo(() => {
  ...
  return (
    <LoaderContainer>
      <KeepAwake />
      <Text style={[styles.title, styles.titleUppercase]}>Đang tải game</Text>
      <View>
        <Text style={styles.progressTitle}>
          <Text style={styles.progressName}>{download.fileName}</Text>
          <Text style={styles.progressMemory}>
            {' '}
            [{formatSizeUnits(download.currentBytes || 0)} trên{' '}
        ...
        <Text style={styles.progressSubtitle}>
          Đang tải các tệp game [{numberOfDownloads} trên{' '}
